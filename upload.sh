...
Repo="CAPrivatePod"
RepoList=("CAPrivatePod" "DUSpecs_test" "DUSpecs_gray")
getRepo() {
    echo -e "\n"
    length=${#RepoList[@]}
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' CAA_Category.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" CAA_Category.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' CAA_Category.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" CAA_Category.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' CAA_Category.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" CAA_Category.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' CAA_Category.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' CAA_Category.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" CAA_Category.podspec
...
pod repo push ${Repo} CAA_Category.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
